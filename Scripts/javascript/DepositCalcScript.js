...
let click = false;
  $(".submitButton").click(function(){
    ...
    if ( currency == 0 ) {
      ...
      if (!isTrue) {
          if (inputType == 3) {
              let monthlyPayment = (inputAmount, inputPercent, inputPeriod) => {
                  inputPeriod /= 36500
                  return inputAmount * inputPercent * inputPeriod;
              }

              let output = Math.round(monthlyPayment);

              resultText.css({ "font-size": "13px", "display": "block", "background-color": "yellow", "padding": "5px" });
